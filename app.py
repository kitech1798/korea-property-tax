...
with st.sidebar:
    ...
    # 개편안 조항은 전부 2027.1.1. 이후 시행이다. 2026년에 개편안 트랙으로 조회하면
    # 맞는 규칙이 없어 MissingRule이 난다 — 그건 오류가 아니라 "아직 시행 전"이라는 뜻이다.
    # 조용히 현행법으로 계산하되, 왜 그런지를 화면에 밝힌다.
    effective_track = track
    if track is Track.REFORM and year <= 2026:
        effective_track = Track.CURRENT
        st.info(
            "2026년에는 개편안 조항이 아직 시행되지 않습니다"
            "(대부분 2027.1.1. 이후 납세의무 성립분부터). 현행법 기준으로 계산합니다."
        )
